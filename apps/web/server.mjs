...
const SECRET = process.env.PROX_AGENT_SECRET || "";
...
function proxyLive(client, search) {
  ...
  const upstream = new WebSocket(target, { headers: { "x-prox-secret": SECRET } });
  const queue = [];
...
server.listen(port, hostname, () => console.log(`▸ Prox web on http://${hostname}:${port}`));
