// Camera → JPEG frames, Gemini-Live style. While the camera is on we sample it
// at ~1fps into a small rolling buffer, so the "current view" is ALWAYS a real,
// recent frame. Each turn attaches the latest buffered frame; the `look` tool
// grabs a fresh HIGHER-res frame on demand.
