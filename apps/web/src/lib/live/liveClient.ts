...
export class LiveClient {
  ...
  /** `productSlug` scopes the session to one product's knowledge graph
   *  (null/omitted = master, across all products). */
  connect(chatId: string, productSlug?: string | null) {
    this.chatId = chatId;
    this.productSlug = productSlug ?? null;
  ...
  private open() {
    ...
    const product = this.productSlug ? `&product=${encodeURIComponent(this.productSlug)}` : "";
    const ws = new WebSocket(`${base}/live?chat=${encodeURIComponent(this.chatId)}${product}`);
  ...
  userText(text: string, frames?: { data: string; mime: string }[]) {
