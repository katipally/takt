...
interface LiveState {
  ...
  toolStatus: string; // active tool name while a tool is running (""), drives the live "Searching the product…" cue
  warming: boolean;   // true from socket-open until the agent signals warm-ready → shows "Warming up…"
  overlay: LiveOverlay | null; // the one pinned visual (OverlayLayer / NotePin)
  error?: string;
  micId?: string;
  camId?: string;
  mics: DeviceOpt[];
