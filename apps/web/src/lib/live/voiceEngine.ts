...
export class VoiceEngine {
  ...
  async start(stream: MediaStream) {
    ...
    this.vad = await MicVAD.new({
      model: "v5",
      // Serve the Silero worklet + onnx + ort wasm SAME-ORIGIN from /public/live
      // (vendored from node_modules — same pattern as /vendor/model-viewer). The
      // CDN route tripped CSP on ort's dynamic .mjs import, and same-origin also
      // means the VAD works offline on a HF Space.
      baseAssetPath: "/live/",
      onnxWASMBasePath: "/live/",
