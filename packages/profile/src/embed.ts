...
// ── binary vector store IO (LEGACY — the on-disk flat store) ────────────────
// Superseded by embedGraph + in-DB BLOBs (semanticSearchKg). Kept only until the
// markdown-chunk retrieval path (hybrid.ts) is fully retired.
