...
export const CATALOG = {
  ...
  // full freeform page — the PRIMARY rich deliverable (renders in a sandboxed,
  // full-bleed, auto-sizing iframe with the Takt design system already loaded)
  Page: { description: "A FULL freeform HTML page — the PRIMARY way to deliver a rich, designed answer that uses the WHOLE canvas (editorial layout, real composition — not a stacked column). `html` is the page body; write semantic HTML/CSS and lay it out with CSS grid/flex. The Takt design-system tokens + base classes are already loaded (see the design guide). Embed grounded media + citations ONLY through these ISLAND elements (everything else is plain HTML): <takt-cite page=\"NN\" product=\"slug\"></takt-cite> a page-citation chip (opens the exact page); <takt-figure src=\"/assets/…\" caption=\"…\"></takt-figure> a real cropped manual figure; <takt-video src=\"/assets/…\"></takt-video>; <takt-model src=\"/assets/*.glb\"></takt-model> the 3D part; <takt-action id=\"actionId\" value=\"…\">Label</takt-action> a button that submits back to Takt. Only use /assets URLs a tool actually returned — NEVER invent one. Draw charts/diagrams as inline <svg>. Optional `css` holds page-specific styles.", props: z.object({ html: z.string().min(1), css: z.string().optional() }) },
