...
export const DESIGN_GUIDE = `DESIGN — the Takt look is editorial and high-craft. The design system (fonts, colors, classes) is ALREADY loaded; don't re-declare colors or fonts, just compose:
  ...
  • CHARTS / DIAGRAMS: draw inline \`<svg>\` using the design tokens — data \`stroke="var(--takt-accent)"\`, axis lines/labels \`stroke/fill="var(--takt-muted)"\`. NEVER paint a solid (black/dark) background rect — leave it transparent so it reads in light AND dark. A black box = broken.
